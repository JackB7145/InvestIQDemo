...
const SUBMIT_TIMEOUT_MS = 90_000; // 90s — matches backend SLA
...
export default function Home() {
    ...
    const toolHandlers: Record<string, (data: DisplayModule["data"]) => void> = {
        LineGraph: (data) =>
            setDisplayBox((prev) => [
                ...prev,
                LineGraph.fromData(data as Parameters<typeof LineGraph.fromData>[0]),
            ]),
        BarGraph: (data) =>
            setDisplayBox((prev) => [
                ...prev,
                BarGraph.fromData(data as Parameters<typeof BarGraph.fromData>[0]),
            ]),
        ScatterPlot: (data) =>
            setDisplayBox((prev) => [
                ...prev,
                ScatterPlotGraph.fromData(
                    data as Parameters<typeof ScatterPlotGraph.fromData>[0]
                ),
            ]),
    };
    ...
    const handleSubmit = async (prompt: string) => {
        ...
        // Seed the user message and thinking bubble
        ...
        try {
            ...
            await parseStream(response, {
                ...
                onResponse: (text) => {
                    // On first response chunk, convert the thinking bubble → bot message
                    if (!botSeeded) {
                        setChatMessages((prev) => {
                            const updated = [...prev];
                ...
                onDisplay: (modules) => {
                    modules.forEach((module) => {
                        ...
                        else console.warn(`No handler for module type: "${module.type}"`);
                    });
                },
            });
        } catch (error) {
            ...
            if (error && !isAbort) console.error("Chat API error:", error);
        ...
        >
            ...
            <Box sx={{ flex: 1, display: "flex", minHeight: 0, overflow: "hidden" }}>
                ...
                >
                    ...
                    >
                        <InputBox handleSubmit={handleSubmit} disabled={isLoading} />
                    </Box>
                </Box>
